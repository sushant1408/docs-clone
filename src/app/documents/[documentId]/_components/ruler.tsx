...
const Ruler = () => {
  ...
  /** Contraints for markers:
   * 1. should only be moved horizontally
   * 2. left marker can never cross right marker and vice versa
   * 3. there should always be some space between two markers
   */
      ...
      if (container) {
        ...
        if (isDraggingLeft) {
          const maxLeftPosition =
            PAGE_WIDTH - rightMargin - MIN_SPACE_BETWEEN_MARKERS;
          const newLeftPosition = Math.min(rawPosition, maxLeftPosition);

          setLeftMargin(newLeftPosition);
        } else if (isDraggingRight) {
          const maxRightPosition =
            PAGE_WIDTH - (leftMargin + MIN_SPACE_BETWEEN_MARKERS);
...
const Marker = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "absolute left-1/2 top-4 -translate-x-1/2 w-[1px] h-[100vh] bg-[#3b72f6]",
